...
public static class FranchiseMappingExtensions
{
    extension(Franchise franchise)
    {
            ...
            IEnumerable<Franchise> franchises)
        {
            ...
            UpdateItems(
                franchises, franchise.FindFranchise, franchise.AttachFranchise, items, FranchiseItemType.Franchise);
        ...
        private ImmutableList<FranchiseItem> RemoveMissingItems(
        ...
        private void ReplaceTitles(FranchiseRequest request)
        {
    ...
    extension(FranchiseItem? item)
    {
        ...
        public FranchiseItemInfoModel? ToFranchiseItemInfoModel() =>
            item is not null
                ? new(
                    item.ParentFranchise.Id.Value,
                    item.RootFranchiseId.Value,
                    item.SequenceNumber,
                    item.DisplayNumber,
                    item.IsFirst,
                    item.IsLast,
    ...
    extension(FranchiseItem item)
    {
        private Id<Franchise> RootFranchiseId =>
            item.ParentFranchise.FranchiseItem is { } parentItem
                ? parentItem.RootFranchiseId
                : item.ParentFranchise.Id;
    }

    extension(Validated<FranchiseRequest> request)
    {
    ...
    extension(IEnumerable<FranchiseItem> items)
    {
        private ImmutableList<FranchiseItemModel> ToItemModels() =>
            [.. items
