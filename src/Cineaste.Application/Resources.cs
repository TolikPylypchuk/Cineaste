namespace Cineaste.Application;

public static class Resources
{
    public const string Any = nameof(Any);
    public const string List = nameof(List);
    public const string ListItem = nameof(ListItem);

    public const string Franchise = nameof(Franchise);
    public const string FranchiseItems = nameof(FranchiseItems);

    public const string Movie = nameof(Movie);
    public const string MovieKind = nameof(MovieKind);

    public const string Series = nameof(Series);
    public const string Period = nameof(Period);
    public const string SpecialEpisode = nameof(SpecialEpisode);
    public const string SeriesKind = nameof(SeriesKind);

    public const string Poster = nameof(Poster);
}
